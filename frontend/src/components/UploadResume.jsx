...
//         extractedSkills: result.data.extractedSkills || [],
...
//       // ✅ Store data in Redux
//       dispatch(setResumeData(resumePayload));
      

//       setTimeout(() => {
//         console.log("🔍 Redux state after 100ms:", window.__REDUX_DEVTOOLS_EXTENSION__ ? "Check Redux DevTools" : resumeData);
//       }, 100);
...
//     // ✅ Get data from Redux
//     const rawData = resumeData?.originalText;
//     const skills = resumeData?.extractedSkills;
...
//       console.error("❌ Validation failed: Missing or short text");
//       alert("Missing resume text. Please upload your resume first.");
//       return;
//     }

//     if (!skills || skills.length === 0) {
//       console.error("❌ Validation failed: No skills found");
//       alert("No skills found. Please upload your resume first.");
//       return;
//     }

//     try {
//       const payload = {
//         originalText: rawData,
...
//       alert("Failed to save skills: " + (error.response?.data?.error || error.message));
//     }
//   };
...
//                         className="p-2 text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-lg transition-all duration-200 opacity-0 group-hover:opacity-100 cursor-pointer"
//                       >
//                         <X className="w-5 h-5" />
//                       </button>
//                     </div>
...
//                   disabled={uploading}
//                   className={`group relative px-8 py-4 rounded-2xl font-semibold transition-all duration-200 cursor-pointer overflow-hidden ${
//                     uploading
...
//                       Resume uploaded successfully! Found {resumeData?.extractedSkills?.length || 0} skills.
//                     </span>
//                   </div>
//                 </div>
...
//                     className="group relative px-8 py-4 rounded-2xl font-semibold transition-all duration-200 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white hover:scale-105 shadow-lg hover:shadow-green-500/50 overflow-hidden cursor-pointer"
//                   >
//                     <div className="flex items-center gap-3 relative z-10">
...
const UploadResume = () => {
  ...
  const handleFiles = (selectedFiles) => {
    ...
    // ✅ Toast for invalid files
    const invalidCount = fileArray.length - validFiles.length;
    if (invalidCount > 0) {
      toast.error(`${invalidCount} invalid file${invalidCount > 1 ? 's' : ''} removed. Only PDF, DOC, DOCX, and TXT files are allowed.`);
    }
  ...
  const removeFile = (fileId) => {
    setFiles((prev) => prev.filter((file) => file.id !== fileId));
    toast.success('File removed', { duration: 2000 }); // ✅ Toast for file removal
  };
  ...
  const handleUpload = async () => {
    if (files.length === 0) {
      toast.error('Please select at least one file to upload'); // ✅ Validation toast
      return;
    }

    // ✅ Loading toast with ID for dismissal
    const uploadToast = toast.loading('Uploading and analyzing your resume...');
    ...
    try {
      ...
      toast.dismiss(uploadToast); // ✅ Dismiss loading toast
      setUploading(false);
      setUploadSuccess(true);

      // ✅ Success toast with skill count
      const skillCount = resumePayload.extractedSkills.length;
      toast.success(
        `Resume uploaded successfully! Found ${skillCount} skill${skillCount !== 1 ? 's' : ''} 🎉`,
        { duration: 5000 }
      );
  ...
  const handleSaveSkills = async () => {
    ...
    if (!rawData || rawData.trim().length < 10) {
      console.error("❌ Validation failed: Missing or short text");
      toast.error('Missing resume text. Please upload your resume first.'); // ✅ Error toast
      return;
    }

    if (!skills || skills.length === 0) {
      console.error("❌ Validation failed: No skills found");
      toast.error('No skills found. Please upload your resume first.'); // ✅ Error toast
      return;
    }

    // ✅ Loading toast
    const saveToast = toast.loading('Saving your skills...');
    ...
    try {
      ...
      toast.dismiss(saveToast); // ✅ Dismiss loading toast
      toast.success('Skills saved successfully! Redirecting to mentors...', { 
        icon: '✅',
        duration: 3000 
      }); // ✅ Success toast
      ...
      toast.error(`Failed to save skills: ${errorMessage}`);
    }
  };
