    ...
    //               className="group flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-cyan-500/10 to-purple-500/10 hover:from-cyan-500/20 hover:to-purple-500/20 rounded-full border border-slate-700 hover:border-cyan-500/50 transition-all duration-300 cursor-pointer"
    //             >
    //               <div className="relative">
    ...
    //                   <div className="py-2">
    //                     <button
    //                       className="w-full text-left px-4 py-3 text-slate-300 hover:text-white hover:bg-slate-800/50 transition-all duration-200 flex items-center gap-3 group cursor-pointer"
    //                       onClick={() => {
    //                         setModalOpen(false);
    ...
    //                     <button
    //                       className="w-full text-left px-4 py-3 text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-all duration-200 flex items-center gap-3 group cursor-pointer"
...
const Nav = () => {
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        ...
        dispatch(clearUserData());
        // ✅ Only show toast and redirect if it's actually an auth error
        if (error.response?.status === 401) {
          toast.error('Session expired. Please log in again.'); // ✅ Auth error toast
          navigate("/login");
        }
      } finally {
        setLoadingAuth(false);
  ...
  const handleLogout = async () => {
    ...
    setLoggingOut(true);
    const logoutToast = toast.loading('Logging out...'); // ✅ Loading toast

    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/auth/logout`, {}, { withCredentials: true });
      dispatch(clearUserData());
      
      toast.dismiss(logoutToast); // ✅ Dismiss loading toast
      toast.success('Logged out successfully. See you soon! 👋', { // ✅ Success toast
        duration: 3000,
      });
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (modalOpen && !event.target.closest('.dropdown-container')) {
        setModalOpen(false);
      }
    };
    ...
    <header className="w-full h-[72px] fixed top-0 left-0 bg-slate-900/80 backdrop-blur-xl border-b border-slate-800 flex items-center justify-center z-50 shadow-lg">
      <nav className="flex justify-between items-center w-[92%] max-w-[1400px]">
        ...
        <Link 
          ...
          onClick={() => toast('Going to dashboard...', { icon: '🏠', duration: 2000 })} // ✅ Navigation toast
        >
          <div className="relative">
            <Sparkles className="w-8 h-8 text-cyan-400 group-hover:rotate-12 transition-transform duration-300" />
            <div className="absolute inset-0 bg-cyan-400 blur-xl opacity-50 group-hover:opacity-75 transition-opacity"></div>
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Avatar Dropdown */}
          <div className="relative dropdown-container"> {/* ✅ Added class for click-outside detection */}
            <button
              onClick={() => setModalOpen(!modalOpen)}
              className="group flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-cyan-500/10 to-purple-500/10 hover:from-cyan-500/20 hover:to-purple-500/20 rounded-full border border-slate-700 hover:border-cyan-500/50 transition-all duration-300 cursor-pointer"
            >
                ...
                >
                  ...
                  <div className="py-2">
                    <button
                      ...
                      onClick={() => {
                        setModalOpen(false);
                        toast('Profile feature coming soon! 🚀', { // ✅ Feature toast
                          icon: '👤',
                          duration: 3000
                        });
                    ...
                    <button
                      className="w-full text-left px-4 py-3 text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-all duration-200 flex items-center gap-3 group cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                      onClick={() => {
                        setModalOpen(false);
                        handleLogout();
                      }}
                    ...
                    >
                      ...
                      <span className="font-medium">
                        {loggingOut ? 'Logging out...' : 'Logout'} {/* ✅ Dynamic text */}
                      </span>
                    </button>
                  </div>
                </motion.div>
